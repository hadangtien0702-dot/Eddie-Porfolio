...
const CARD_H = 380;
const OFFSET_X_DESKTOP = 300;
const OFFSET_X_MOBILE = 140;
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Header */}
          <div className="relative z-30 flex items-center justify-between px-8 py-4 md:px-12 md:py-4">
          ...
          {activeCategory === "Ads Performance" ? (
            /* Main Carousel Area */
            <div className="relative flex-1 flex flex-col items-center justify-center pt-2 pb-8 min-h-0" style={{ perspective: "1200px" }}>
              <motion.div
              ...
              <div className="relative flex items-center justify-center w-full" style={{ height: isMobile ? 360 : 490, transformStyle: "preserve-3d" }}>
                <AnimatePresence mode="popLayout">
                  {filteredVideos.map((video, i) => {
                    ...
                    const isCenter = offset === 0;
                       ...
                       if (isMobile) {
                         if (ratio === "16:9") return { w: "w-[260px]", h: "h-[146px]" };
                         if (ratio === "1:1") return { w: "w-[200px]", h: "h-[200px]" };
                         if (ratio === "4:5") return { w: "w-[200px]", h: "h-[250px]" };
                         return { w: "w-[180px]", h: "h-[320px]" };
                       }
                       if (ratio === "16:9") return { w: "w-[640px]", h: "h-[360px]" }; // Cinematic 16:9
                       if (ratio === "1:1") return { w: "w-[400px]", h: "h-[400px]" }; // Large Square
                       if (ratio === "4:5") return { w: "w-[340px]", h: "h-[425px]" }; // Professional Vertical
                       return { w: "w-[258px]", h: "h-[460px]" };
                     };
              ...
              {/* Navigation Controls */}
              <div className="relative z-30 flex items-center justify-center gap-6 py-2 mt-4">
                <button onClick={() => goTo(activeIndex - 1)} className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/30 hover:text-white hover:border-white/20 transition-all">
                  <ChevronLeft size={16} />
                </button>
                <div className="flex gap-1.5">
                  {filteredVideos.map((_, i) => (
                    <button key={i} onClick={() => goTo(i)} className={`h-1.5 rounded-full transition-all duration-500 ${i === activeIndex ? 'w-5 bg-accent' : 'w-1.5 bg-white/10'}`} />
                  ))}
                </div>
                <button onClick={() => goTo(activeIndex + 1)} className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/30 hover:text-white hover:border-white/20 transition-all">
              ...
              {/* Integrated Info + Detailed Metrics Panel */}
              <div className="relative z-30 w-full max-w-3xl px-6 md:px-8 mt-6">
                <div className="w-full bg-white/[0.02] border border-white/10 rounded-[24px] p-6 md:p-8 backdrop-blur-3xl relative overflow-hidden group hover:border-accent/20 transition-all duration-500 shadow-2xl">
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {/* Upper row: Title, badge, description */}
                      <div className="space-y-2 text-left">
                        <div className="flex items-center gap-2.5">
                          <span className="px-2 py-0.5 bg-accent text-[8.5px] font-mono font-black text-white uppercase tracking-widest rounded-sm">
                            {currentVideo?.year || "2024"}
                          </span>
                          <span className="font-mono text-[9px] text-accent/80 uppercase tracking-[0.2em] font-black">
                            {currentVideo?.role}
                          </span>
                        </div>
                        
                        <h2 className="font-heading text-lg md:text-xl lg:text-2xl font-black text-white uppercase tracking-tighter leading-tight">
                          {currentVideo?.title}
                        </h2>
                        
                        <p className="text-white/45 text-[11px] md:text-xs leading-relaxed font-medium">
                      ...
                      {/* Lower row: Stats Grid */}
                      <div className="grid grid-cols-3 gap-4 items-center">
                        {/* CPA Metric */}
                        <div className="relative space-y-1 text-left">
                          <div className="flex items-center gap-1.5">
                            <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse shadow-[0_0_10px_#ff4000]" />
                            <span className="font-mono text-[8px] text-white/50 uppercase tracking-[0.25em] font-black">CPA</span>
                          </div>
                          <div className="flex items-baseline gap-1">
                            <span className="text-xl md:text-2xl lg:text-3xl font-black text-white tracking-tighter drop-shadow-[0_0_15px_rgba(255,64,0,0.2)]">
                              {currentVideo?.stats.cpa || "$--" }
                            </span>
                            <span className="text-[7px] font-mono text-accent font-black uppercase bg-accent/10 px-1.5 py-0.5 rounded border border-accent/15">MET</span>
                          </div>
                        </div>

                        {/* Hook Retention Gauge */}
                        <div className="relative flex flex-col items-center border-x border-white/5 px-2">
                          <span className="font-mono text-[8px] text-white/50 uppercase tracking-[0.25em] font-black mb-1">HOOK</span>
                          <div className="text-xl md:text-2xl lg:text-3xl font-black text-white leading-none">
                            {currentVideo?.stats.retention3s || "0%" }
                          </div>
                          <span className="text-[7.5px] text-white/25 uppercase tracking-tighter italic mt-1">First 5s</span>
                        </div>

                        {/* Views Reach */}
                        <div className="relative flex flex-col items-end">
                          <span className="font-mono text-[8px] text-white/50 uppercase tracking-[0.25em] font-black mb-1">REACH</span>
                          <div className="text-xl md:text-2xl lg:text-3xl font-black text-white flex items-center gap-1.5">
